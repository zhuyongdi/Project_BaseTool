package com.zhuyongdi.app.decoration;
...
import com.zhuyongdi.app.decoration.provider.ZYDDefaultGridProvider;
import com.zhuyongdi.app.decoration.provider.ZYDIGridProvider;
 ...
 * @author dkzwm
 */
public class ZYDGridItemDecoration extends ZYDBaseItemDecoration<ZYDIGridProvider> {

    private ZYDGridItemDecoration(Builder builder) {
    ...
    void calculateItemOffsets(RecyclerView parent, int position, Rect rect) {
        ...
        if (manager.getOrientation() == OrientationHelper.VERTICAL) {
            ...
            boolean drawRow = isRowNeedDraw(row, totalSpanSize, true, manager.getReverseLayout(),
                    spanCount);
            if (drawRow) {
                ZYDIDivider rowDivider = createRowDivider(row, true,
            ...
            if (manager.getReverseLayout()) {
                ...
                if (drawColumn) {
                    ZYDIDivider columnDivider = createColumnDivider(column, true, true);
                ...
                if (drawColumn) {
                    ZYDIDivider columnDivider = createColumnDivider(column, true, false);
                    columnSize = columnDivider.getDividerSize();
                }
            ...
            boolean drawRow;
            if (drawColumn) {
                ZYDIDivider columnDivider = createColumnDivider(column, false,
                        manager.getReverseLayout());
                columnSize = columnDivider.getDividerSize();
            }
            if (manager.getReverseLayout()) {
                ...
                if (drawRow) {
                    ZYDIDivider rowDivider = createRowDivider(row, false, true);
                    rowSize = rowDivider.getDividerSize();
                }
                rect.set(0, 0, columnSize, rowSize);
            } else {
                ...
                if (drawRow) {
                    ZYDIDivider rowDivider = createRowDivider(row, false, false);
                    rowSize = rowDivider.getDividerSize();
                }
                rect.set(columnSize, rowSize, 0, 0);
            }
    ...
    void drawVerticalOrientationDividers(Canvas c,
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (drawColumn) {
                ...
                if (manager.getReverseLayout()) {
                    ZYDIDivider columnDivider = createColumnDivider(column, true, true);
                    if (columnDivider.getType() == ZYDIDivider.TYPE_DRAWABLE) {
                    ...
                    if (needDrawLastRow) {
                        left = view.getRight() + lp.rightMargin + transitionX;
                        right = left + columnDivider.getDividerSize();
                        ZYDIDivider rowDivider = mProvider.createRowDivider(row - 1);
                        if (rowDivider.getType() == ZYDIDivider.TYPE_DRAWABLE) {
                            top = bottom;
            ...
            if (drawRow) {
                ZYDIDivider rowDivider = createRowDivider(row, true, manager.getReverseLayout());
                left = view.getLeft() - lp.leftMargin + transitionX;
    ...
    void drawHorizontalOrientationDividers(Canvas c,
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (drawRow) {
                ZYDIDivider rowDivider = createRowDivider(row, false, manager
                        .getReverseLayout());
                left = view.getLeft() - lp.leftMargin + transitionX;
                right = view.getRight() + lp.rightMargin + transitionX;
                if (manager.getReverseLayout()) {
                    ...
                    if (needDrawLastColumn) {
                        top = view.getBottom() + lp.bottomMargin + transitionY;
                        bottom = top + rowDivider.getDividerSize();
                        ZYDIDivider columnDivider = mProvider.createColumnDivider(column - 1);
                        if (columnDivider.getType() == ZYDIDivider.TYPE_DRAWABLE) {
                            left = right;
                            right = left + columnDivider.getDividerSize();
                        } else {
    ...
    private ZYDIDivider createColumnDivider(int column,
                                            boolean vertical,
    ...
    private ZYDIDivider createRowDivider(int row,
                                         boolean vertical,
                                         boolean reverseLayout) {
        if (vertical) {
    ...
    public static class Builder implements IBuilder<Builder, ZYDIGridProvider> {
        Context mContext;
        ZYDIGridProvider mProvider;
        ...
        @Override
        public Builder provider(@NonNull ZYDIGridProvider provider) {
            if (mProvider != null) {
        ...
        public Builder rowDivider(int row, ZYDIDivider divider) {
            checkProvider();
            ((ZYDDefaultGridProvider) mProvider).setRowDivider(row, divider);
            return this;
        }

        public Builder rowDivider(ZYDIDivider divider) {
            checkProvider();
            ((ZYDDefaultGridProvider) mProvider).setAllRowDivider(divider);
            return this;
        }

        public Builder columnDivider(int column, ZYDIDivider divider) {
            checkProvider();
            ((ZYDDefaultGridProvider) mProvider).setColumnDivider(column, divider);
            return this;
        }

        public Builder columnDivider(ZYDIDivider divider) {
            checkProvider();
            ((ZYDDefaultGridProvider) mProvider).setAllColumnDivider(divider);
            return this;
        }

        public Builder rowNeedDraw(int row, boolean need) {
            checkProvider();
            ((ZYDDefaultGridProvider) mProvider).setRowNeedDraw(row, need);
            return this;
        }

        public Builder columnNeedDraw(int column, boolean need) {
            checkProvider();
            ((ZYDDefaultGridProvider) mProvider).setColumnNeedDraw(column, need);
            return this;
        }
        ...
        private void checkProvider() {
            checkProviderIsNull();
            if (!(mProvider instanceof ZYDDefaultGridProvider))
                mProvider = new ZYDDefaultGridProvider(mProvider);
        }

        private void checkProviderIsNull() {
            if (mProvider == null)
                mProvider = new ZYDDefaultGridProvider();
        }

        public ZYDGridItemDecoration build() {
            checkProviderIsNull();
            return new ZYDGridItemDecoration(this);
        }
    }
}
