...
public class XXColorDivider implements XXIDivider {
    ...
    public XXColorDivider() {
        this(DEFAULT_COLOR, DEFAULT_SIZE);
    }

    public XXColorDivider(@ColorInt int color) {
        this(color, DEFAULT_SIZE);
    }

    public XXColorDivider(@ColorInt int color, int size) {
        mColor = color;
        mSize = size;
