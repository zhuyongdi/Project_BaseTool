...
import com.zhuyongdi.basetool.function.screen_adaption_ctx.unit.XXUnitsManager;
...
import com.zhuyongdi.basetool.function.screen_adaption_ctx.utils.XXScreenUtils;
 ...
 * ================================================
 */
public final class XXAutoSizeConfig {
    private static volatile XXAutoSizeConfig sInstance;
     ...
     */
    private XXExternalAdaptManager mExternalAdaptManager = new XXExternalAdaptManager();
    /**
     * 用来管理 AndroidAutoSize 支持的所有单位, AndroidAutoSize 支持五种单位 (dp、sp、pt、in、mm)
     */
    private XXUnitsManager mUnitsManager = new XXUnitsManager();
    /**
     ...
     * AndroidAutoSize 默认使用 {@link XXScreenUtils#getStatusBarHeight()} 方法获取状态栏高度
     ...
     */
    private XXActivityLifecycleCallbacksImpl mActivityLifecycleCallbacks;
    /**
     ...
     */
    private XXOnAdaptListener mOnAdaptListener;

    public static XXAutoSizeConfig getInstance() {
        if (sInstance == null) {
            synchronized (XXAutoSizeConfig.class) {
                if (sInstance == null) {
                    sInstance = new XXAutoSizeConfig();
                }
            }
        }
        return sInstance;
    ...
    private XXAutoSizeConfig() {
    }

    public Application getApplication() {
        XXPreconditions.checkNotNull(mApplication, "Please call the AutoSizeConfig#init() first");
        return mApplication;
    }
     ...
     */
    XXAutoSizeConfig init(Application application) {
        return init(application, true, null);
    }

    /**
     * v0.7.0 以后, 框架会在 APP 启动时自动调用此方法进行初始化, 使用者无需手动初始化, 初始化方法只能调用一次, 否则报错
     * 此方法使用默认的 {@link XXAutoAdaptStrategy} 策略, 如想使用自定义的 {@link XXAutoAdaptStrategy} 策略
     * 请调用 {@link #init(Application, boolean, XXAutoAdaptStrategy)}
     *
     * @param application   {@link Application}
     * @param isBaseOnWidth 详情请查看 {@link #isBaseOnWidth} 的注释
     */
    XXAutoSizeConfig init(Application application, boolean isBaseOnWidth) {
     ...
     * @param application   {@link Application}
     * @param isBaseOnWidth 详情请查看 {@link #isBaseOnWidth} 的注释
     * @param strategy      {@link XXAutoAdaptStrategy}, 传 {@code null} 则使用 {@link XXDefaultAutoAdaptStrategy}
     */
    XXAutoSizeConfig init(final Application application, boolean isBaseOnWidth, XXAutoAdaptStrategy strategy) {
        XXPreconditions.checkArgument(mInitDensity == -1, "AutoSizeConfig#init() can only be called once");
        XXPreconditions.checkNotNull(application, "application == null");
        ...
        int[] screenSize = XXScreenUtils.getScreenSize(application);
        mScreenWidth = screenSize[0];
        mScreenHeight = screenSize[1];
        mStatusBarHeight = XXScreenUtils.getStatusBarHeight();
        XXLogUtils.d("designWidthInDp = " + mDesignWidthInDp + ", designHeightInDp = " + mDesignHeightInDp + ", screenWidth = " + mScreenWidth + ", screenHeight = " + mScreenHeight);
            ...
            public void onConfigurationChanged(Configuration newConfig) {
                if (newConfig != null) {
                    if (newConfig.fontScale > 0) {
                        ...
                        XXLogUtils.d("initScaledDensity = " + mInitScaledDensity + " on ConfigurationChanged");
                    }
        ...
        XXLogUtils.d("initDensity = " + mInitDensity + ", initScaledDensity = " + mInitScaledDensity);
        mActivityLifecycleCallbacks = new XXActivityLifecycleCallbacksImpl(strategy == null ? new XXWrapperAutoAdaptStrategy(new XXDefaultAutoAdaptStrategy()) : strategy);
     ...
     */
    public void restart() {
        XXPreconditions.checkNotNull(mActivityLifecycleCallbacks, "Please call the AutoSizeConfig#init() first");
        synchronized (XXAutoSizeConfig.class) {
            if (isStop) {
     ...
     * 设置屏幕适配逻辑策略类
     *
     * @param autoAdaptStrategy {@link XXAutoAdaptStrategy}
     */
    public XXAutoSizeConfig setAutoAdaptStrategy(XXAutoAdaptStrategy autoAdaptStrategy) {
        XXPreconditions.checkNotNull(autoAdaptStrategy, "autoAdaptStrategy == null");
        XXPreconditions.checkNotNull(mActivityLifecycleCallbacks, "Please call the AutoSizeConfig#init() first");
        mActivityLifecycleCallbacks.setAutoAdaptStrategy(new XXWrapperAutoAdaptStrategy(autoAdaptStrategy));
        return this;
    }
     ...
     * 设置屏幕适配监听器
     *
     * @param onAdaptListener {@link XXOnAdaptListener}
     */
    public XXAutoSizeConfig setOnAdaptListener(XXOnAdaptListener onAdaptListener) {
        XXPreconditions.checkNotNull(onAdaptListener, "onAdaptListener == null");
     ...
     */
    public XXAutoSizeConfig setBaseOnWidth(boolean baseOnWidth) {
     ...
     */
    public XXAutoSizeConfig setUseDeviceSize(boolean useDeviceSize) {
     ...
     */
    public XXAutoSizeConfig setLog(boolean log) {
        XXLogUtils.setDebug(log);
        return this;
    }
     ...
     */
    public XXAutoSizeConfig setCustomFragment(boolean customFragment) {
    ...
    /**
     * {@link XXExternalAdaptManager} 用来管理外部三方库 {@link Activity} 的适配
     *
     * @return {@link #mExternalAdaptManager}
     */
    public XXExternalAdaptManager getExternalAdaptManager() {
        return mExternalAdaptManager;
    }

    /**
     * {@link XXUnitsManager} 用来管理 AndroidAutoSize 支持的所有单位, AndroidAutoSize 支持五种单位 (dp、sp、pt、in、mm)
     *
     * @return {@link #mUnitsManager}
     */
    public XXUnitsManager getUnitsManager() {
     ...
     */
    public XXOnAdaptListener getOnAdaptListener() {
    ...
    public int getDesignWidthInDp() {
        XXPreconditions.checkArgument(mDesignWidthInDp > 0, "you must set " + KEY_DESIGN_WIDTH_IN_DP + "  in your AndroidManifest file");
    ...
    public int getDesignHeightInDp() {
        XXPreconditions.checkArgument(mDesignHeightInDp > 0, "you must set " + KEY_DESIGN_HEIGHT_IN_DP + "  in your AndroidManifest file");
     ...
     */
    public XXAutoSizeConfig setVertical(boolean vertical) {
        isVertical = vertical;
        return this;
    }
     ...
     */
    public XXAutoSizeConfig setExcludeFontScale(boolean excludeFontScale) {
     ...
     */
    public XXAutoSizeConfig setScreenWidth(int screenWidth) {
        XXPreconditions.checkArgument(screenWidth > 0, "screenWidth must be > 0");
     ...
     */
    public XXAutoSizeConfig setScreenHeight(int screenHeight) {
        XXPreconditions.checkArgument(screenHeight > 0, "screenHeight must be > 0");
     ...
     */
    public XXAutoSizeConfig setDesignWidthInDp(int designWidthInDp) {
        XXPreconditions.checkArgument(designWidthInDp > 0, "designWidthInDp must be > 0");
     ...
     */
    public XXAutoSizeConfig setDesignHeightInDp(int designHeightInDp) {
        XXPreconditions.checkArgument(designHeightInDp > 0, "designHeightInDp must be > 0");
     ...
     */
    public XXAutoSizeConfig setStatusBarHeight(int statusBarHeight) {
        XXPreconditions.checkArgument(statusBarHeight > 0, "statusBarHeight must be > 0");
