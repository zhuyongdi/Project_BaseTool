...
/**
 * ================================================
 * {@link XXExternalAdaptInfo} 用来存储外部三方库的适配参数, 因为 AndroidAutoSize 默认会对项目中的所有模块都进行适配
 ...
 * ================================================
 */
public class XXExternalAdaptInfo implements Parcelable {
    /**
    ...
    public XXExternalAdaptInfo(boolean isBaseOnWidth) {
        this.isBaseOnWidth = isBaseOnWidth;
    }

    public XXExternalAdaptInfo(boolean isBaseOnWidth, float sizeInDp) {
        this.isBaseOnWidth = isBaseOnWidth;
    ...
    protected XXExternalAdaptInfo(Parcel in) {
        this.isBaseOnWidth = in.readByte() != 0;
        this.sizeInDp = in.readFloat();
    }

    public static final Creator<XXExternalAdaptInfo> CREATOR = new Creator<XXExternalAdaptInfo>() {
        @Override
        public XXExternalAdaptInfo createFromParcel(Parcel source) {
            return new XXExternalAdaptInfo(source);
        }

        @Override
        public XXExternalAdaptInfo[] newArray(int size) {
            return new XXExternalAdaptInfo[size];
        }
    };
