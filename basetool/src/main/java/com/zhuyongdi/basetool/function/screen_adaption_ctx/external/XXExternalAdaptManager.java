 ...
 */
package com.zhuyongdi.basetool.function.screen_adaption_ctx.external;
 ...
 * {@link XXExternalAdaptManager} 就是专门用来处理这个问题, 项目初始化时把对应的三方库 {@link Activity} 传入 {@link XXExternalAdaptManager} 即可
 * <p>
 ...
 * ================================================
 */
public class XXExternalAdaptManager {
    private List<String> mCancelAdaptList;
    private Map<String, XXExternalAdaptInfo> mExternalAdaptInfos;
     ...
     * <p>
     * 支持链式调用, 如:
     * {@link XXExternalAdaptManager#addCancelAdaptOfActivity(Class)#addCancelAdaptOfActivity(Class)}
     *
     * @param targetClass {@link Activity} class, {@link Fragment} class
     */
    public synchronized XXExternalAdaptManager addCancelAdaptOfActivity(Class<?> targetClass) {
        XXPreconditions.checkNotNull(targetClass, "targetClass == null");
        if (!isRun) {
            isRun = true;
        }
     ...
     * <p>
     * 支持链式调用, 如:
     * {@link XXExternalAdaptManager#addExternalAdaptInfoOfActivity(Class, XXExternalAdaptInfo)#addExternalAdaptInfoOfActivity(Class, XXExternalAdaptInfo)}
     *
     * @param targetClass {@link Activity} class, {@link Fragment} class
     * @param info        {@link XXExternalAdaptInfo} 适配参数
     */
    public synchronized XXExternalAdaptManager addExternalAdaptInfoOfActivity(Class<?> targetClass, XXExternalAdaptInfo info) {
        XXPreconditions.checkNotNull(targetClass, "targetClass == null");
        if (!isRun) {
            isRun = true;
        }
     ...
     */
    public synchronized XXExternalAdaptInfo getExternalAdaptInfoOfActivity(Class<?> targetClass) {
        XXPreconditions.checkNotNull(targetClass, "targetClass == null");
        if (mExternalAdaptInfos == null) {
            return null;
        }
     ...
     */
    public XXExternalAdaptManager setRun(boolean run) {
        isRun = run;
        return this;
    }
}
