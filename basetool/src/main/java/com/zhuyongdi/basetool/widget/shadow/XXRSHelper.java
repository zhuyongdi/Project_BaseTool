...
public class XXRSHelper {
    ...
    public void initAttrs(Context context, AttributeSet attrs) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.XXRSLayout);
        mStrokeWidth = ta.getDimensionPixelSize(R.styleable.XXRSLayout_xx_rs_stroke_width, 0);
        mStrokeColor = ta.getColor(R.styleable.XXRSLayout_xx_rs_stroke_color, 0xFF333333);
        int radius = ta.getDimensionPixelSize(R.styleable.XXRSLayout_xx_rs_radius, 0);
        mShadowColor = ta.getColor(R.styleable.XXRSLayout_xx_rs_shadow_color, Color.GRAY);
        mShadowRadius = ta.getDimensionPixelSize(R.styleable.XXRSLayout_xx_rs_shadow_width, 20);
        mShadowOffsetX = ta.getDimensionPixelSize(R.styleable.XXRSLayout_xx_rs_shadow_offset_x, 0);
        mShadowOffsetY = ta.getDimensionPixelSize(R.styleable.XXRSLayout_xx_rs_shadow_offset_y, 0);
        int leftTopRadius = ta.getDimensionPixelSize(
                R.styleable.XXRSLayout_xx_rs_radius_left_top, radius);
        int leftBottomRadius = ta.getDimensionPixelSize(
                R.styleable.XXRSLayout_xx_rs_radius_left_bottom, radius);
        int rightTopRadius = ta.getDimensionPixelSize(
                R.styleable.XXRSLayout_xx_rs_radius_right_top, radius);
        int rightBottomRadius = ta.getDimensionPixelSize(
                R.styleable.XXRSLayout_xx_rs_radius_right_bottom, radius);
        ta.recycle();
